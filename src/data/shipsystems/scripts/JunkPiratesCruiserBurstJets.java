...
public class JunkPiratesCruiserBurstJets extends BaseShipSystemScript {
    ...
    public static final float SPEED_BONUS = 80f;
    public static final float ACCEL_BONUS = 250f;
    public static final float DECEL_BONUS = 150f;
    public static final float TURN_ACCEL_BONUS = 100f;
    public static final float TURN_RATE_BONUS = 20f;
			...
			if (engines != null && !engines.isEmpty()) {
				...
				for (int i = 0; i < count; i++) {
					...
					if (engine != null && engine.getEngineSlot() != null && engine.getLocation() != null) {
						float nozzleAngle = Misc.normalizeAngle(ship.getFacing() + engine.getEngineSlot().getAngle() + MathUtils.getRandomNumberInRange(-10f, 10f));
	...
	public StatusData getStatusData(int index, State state, float effectLevel) {
		if (index == 0) {
			return new StatusData("cryo-tank burst active", false);
		} else if (index == 1) {
			if (state == State.OUT) return null;
			return new StatusData("+" + (int) SPEED_BONUS + " burst top speed", false);
		}
		return null;
	}
